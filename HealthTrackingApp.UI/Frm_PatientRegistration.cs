...
public partial class Frm_PatientRegistration : Form
{
    ...
    {
        dgvPatientList.Columns["SSN"].HeaderText = "TC Kimlik No";
        dgvPatientList.Columns["FullName"].HeaderText = "Adı Soyadı";
        dgvPatientList.Columns["BirthDate"].HeaderText = "Doğum Tarihi";
        dgvPatientList.Columns["Gender"].HeaderText = "Cinsiyet";
        dgvPatientList.Columns["Phone"].HeaderText = "Telefon";
        dgvPatientList.Columns["Email"].HeaderText = "E-posta";
        dgvPatientList.Columns["Address"].HeaderText = "Adres";
        dgvPatientList.Columns["MaritalStatus"].HeaderText = "Medeni Durum";
        dgvPatientList.Columns["EmergencyContactPerson"].HeaderText = "Acil Durum Kişisi";
        dgvPatientList.Columns["EmergencyContactNumber"].HeaderText = "Acil Durum Telefonu";
        dgvPatientList.Columns["MedicalHistory"].HeaderText = "Tıbbi Geçmiş";
        dgvPatientList.Columns["BloodType"].HeaderText = "Kan Grubu";
        dgvPatientList.Columns["Allergies"].HeaderText = "Alerjiler";
        dgvPatientList.Columns["ChronicDiseases"].HeaderText = "Kronik Hastalıklar";
        dgvPatientList.Columns["Medications"].HeaderText = "Kullanılan İlaçlar";
    }
    ...
    private void btnPatientSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                MessageBox.Show("Lütfen cinsiyet seçin.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (cmbBloodType.SelectedItem == null)
            {
                MessageBox.Show("Lütfen kan grubu seçin.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (cmbInsuranceType.SelectedItem == null)
            {
                MessageBox.Show("Lütfen sigorta türü seçin.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
    ...
    private void Frm_PatientRegistration_FormClosing(object sender, FormClosingEventArgs e)
    {
        Application.Exit();
    }
}
